  ...
  return (
    <div className={cn("inline-flex items-center gap-2.5 select-none group", className)}>
      ...
      <svg
        ...
        className={cn("h-6 sm:h-7 w-auto shrink-0 transition-transform duration-300 group-hover:scale-105", iconClassName)}
      >
        <defs>
        ...
        <path
          ...
          fill={isWhite ? "#FFFFFF" : "url(#brandLogoGrad)"}
        />
      </svg>
      ...
      {showText && (
        <span
          className={cn(
            ...
            isWhite ? "text-white" : isLight ? "text-white" : "text-zinc-950",
            textClassName
          )}
        >
          <span>Stable</span>
          <span className={isWhite ? "text-white" : "text-brand-purple"}>Bank</span>
          {!isWhite && <span className="inline-block h-2 w-2 rounded-full bg-[#B0BE19] ml-1 self-center shadow-xs" />}
        </span>
      )}
    </div>
  );
